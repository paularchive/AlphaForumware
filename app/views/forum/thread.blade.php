...
@section('content')
	...
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4>{{ $thread->title }}<br><small>by <strong>{{ $author }}</strong> &raquo; <em>{{ $thread->created_at }}</em></small></h4>
		</div>
		<div class="panel-body">
			{{ BBCode::parse($thread->body) }}
			@if($thread->updated_at != $thread->created_at)
			<br><small>Last edited by <strong>{{ $author }}</strong> on <em>{{ $thread->updated_at }}</em></small>
			@endif
		</div>
	</div>
	...
	@foreach($thread->comments()->get() as $comment)
		<div class="panel panel-default">
			<div class="panel-heading clearfix">
				<h4 class="pull-left">Re: {{ $comment->thread->title }}<br><small>by <strong>{{ $comment->author->username }}</strong> &raquo; <em>{{ $comment->created_at }}</em></small></h4>
				@if(Auth::check() && Auth::user()->isAdmin() || Auth::check() && Auth::user()->id == $thread->author_id)
					<div class="dropdown pull-right">
						<button id="options-menu" type="button" class="btn btn-default btn-xs" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Options <span class="caret"></span></button>
						<ul class="dropdown-menu" role="menu" aria-labelledby="options-menu">
			...
			<div class="panel-body">
				...
				<br><small>Last edited by <strong>{{ $comment->author->username }}</strong> on <em>{{ $comment->updated_at }}</em></small>
				@endif
			</div>
		</div>
	@endforeach
