...
<div class="sixteen wide column">
	<div class="ui breadcrumb segment" style="width: 100%;">
		...
		<a href="{{ URL::route('forum-category', $reply->category->slug) }}" class="section">{{ $reply->category->title }}</a>
		<i class="right chevron icon divider"></i>
		<a href="{{ URL::route('forum-sub-category', $reply->subcategory->slug) }}" class="section">{{ $reply->subcategory->title }}</a>
		<i class="right chevron icon divider"></i>
		<a href="{{ URL::route('forum-thread', $reply->thread->slug) }}" class="section">{{ $reply->thread->title }}</a>
		<i class="right chevron icon divider"></i>
		<div class="active section">Edit Reply </div>
	</div>
</div>
<div class="sixteen wide column">
	<div class="ui column form{{ ($errors->has()) ? ' error' : '' }}">
		...
		{{ Form::model($reply, array('url' => 'forum/topic/'.$reply->thread->slug.'/reply?edit='.$reply->id, 'id' => 'postform')) }}    

			<div class="field{{ ($errors->has('title')) ? ' error' : '' }}">
				<label for="username">Subject: </label>
				{{ Form::text('title', 'Re: '.$reply->thread->title) }}
			</div>
		    ...
		    <p>{{ Form::submit('Save Reply', array('class' => 'ui button teal')) }}</p>
