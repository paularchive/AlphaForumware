...
@section('content')
	<ol class="breadcrumb">
		...
		<li><a href="{{ URL::route('forum-category', $comment->category->id) }}">{{ $comment->category->title }}</a></li>
	...
	{{ Form::model($comment, array('route' => array('forum-edit-comment', $comment->id), 'id' => 'postform')) }}    
	    ...
	    <p>{{ Form::submit('Save Comment', array('class' => 'btn btn-primary')) }}<p>
